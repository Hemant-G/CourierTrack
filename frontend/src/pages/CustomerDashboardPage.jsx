      ...
      <div className="container mx-auto p-6">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            <h2 className="text-2xl font-semibold mb-4 text-gray-800">My Current Packages</h2>
            {packages.length === 0 ? (
              <div className="text-center bg-blue-50 text-blue-800 rounded-lg p-4">
              ...
              <div className="space-y-4">
                {packages.map((pkg) => (
                  <div key={pkg._id} className="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-100">
                    <h3 className="text-lg font-bold text-blue-700">Tracking ID: {pkg.trackingId}</h3>
                    ...
                    <p className="text-gray-700 text-sm">
                      <strong className="text-gray-900">From:</strong> {pkg.senderInfo.name}
                    </p>
                    <p className="text-gray-700 text-sm">
                      <strong className="text-gray-900">To:</strong> {pkg.recipientInfo.name}
                    </p>
                    {pkg.currentLocation && (
                      <p className="text-gray-700 text-sm">
                        <strong className="text-gray-900">Current Location:</strong> {pkg.currentLocation}
                      </p>
                    )}
                    {pkg.eta && (
                      <p className="text-gray-700 text-sm">
                        <strong className="text-gray-900">ETA:</strong> {format(new Date(pkg.eta), 'PPP p')}
                      </p>
                    )}
                    <Link
                        to={`/track-package?trackingId=${pkg.trackingId}`} // Link to the public tracking page
                        className="mt-3 inline-block bg-blue-500 hover:bg-blue-600 text-white font-bold py-1.5 px-4 rounded text-sm transition duration-150"
                    >
                        View Details
                    </Link>
                  </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 flex flex-col justify-between items-center text-center">
            <h2 className="text-2xl font-semibold mb-4 text-gray-800">Send New Package</h2>
